...
public:
	...
	double Diag()
	{
		return sqrt((max[0] - min[0])*(max[0] - min[0]) +
			(max[1] - min[1])*(max[1] - min[1]) +
			(max[2] - min[2])*(max[2] - min[2]));
	}
	double DimX() { return max[0] - min[0]; }
	double DimY() { return max[1] - min[1]; }
	double DimZ() { return max[2] - min[2]; }
